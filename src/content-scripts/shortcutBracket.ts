...
export const controlBracket = (event: KeyboardEvent) => {
    ...
    const updateText = (position: TextPosition, isTextSelected: boolean) => {
        ...
        if (targetText !== bracketedText) {
            target.value = bracketedText;
    ...
    const leftText = targetText.substring(0, caretPosition);
    const rightText = targetText.substring(caretPosition);
    ...
    const charBeforeCaret = targetText[caretPosition - 1];
    const charAfterCaret = targetText[caretPosition];
